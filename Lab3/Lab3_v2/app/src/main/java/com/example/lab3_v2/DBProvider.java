    ...
    class DBHelper extends SQLiteOpenHelper
    {
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion)
        {
            if (oldVersion == 1 && newVersion == 2)
            {
                ...
                Cursor cursor = db.rawQuery("select * from students_backup", null);
                ...
                while (cursor.moveToNext())
                {
                    ...
                    db.execSQL(
                        "insert into students (" +
                            "id, first_name, last_name, patronymic, date_add" +
                            ") VALUES (?, ?, ?, ?, ?)",
                        new String[]{
                            cursor.getString(cursor.getColumnIndex("id")),
                            full_name[1], full_name[0], full_name[2],
                            String.valueOf(Timestamp.valueOf(cursor.getString(cursor.getColumnIndex("date_add"))))
                        }
                    );
                }
